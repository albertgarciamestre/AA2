    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if location.y < borderBottom.position.y {
                let pointerPath: CGMutablePath = CGMutablePath()
                pointerPath.move(to: ballStartingLocation.position)
                pointerPath.addLine(to: CGPoint(x: ballStartingLocation.position.x - GamePointer.multiplier * (location.x - ballStartingLocation.position.x),
                                                y: ballStartingLocation.position.y - GamePointer.multiplier * (location.y - ballStartingLocation.position.y)))

                let newPointer = GamePointer(newPath: pointerPath)
